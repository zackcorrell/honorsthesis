...
class MrIcpPlugin : public Driver
 {
	...
	// Constructor
	public:  	MrIcpPlugin(ConfigFile* cf, int section); 
...
Driver* MrIcpPlugin_Init(ConfigFile* cf, int section) // Create and return a new instance of this driver
{
  	return ((Driver*) (new MrIcpPlugin(cf, section)));
}

void MrIcpPlugin_Register(DriverTable* table)
{
  	table->AddDriver("mricpplugin", MrIcpPlugin_Init);
}

MrIcpPlugin::MrIcpPlugin(ConfigFile* cf, int section)  : Driver(cf, section)
{
	char config_temp[40];
...
int MrIcpPlugin::Setup()
{
	char filename[40],command[40];
	printf("\n- Setting UP MrIcpPlugin Plugin Driver.");
...
void MrIcpPlugin::SetupPositionDriver()
{
...
int MrIcpPlugin::SetupLaser(int index)
{
...
int MrIcpPlugin::Shutdown()
{
	// Stop and join the driver thread
	cout<<"\n- Shutting Down MrIcpPlugin Driver - Cleaning up Mess ..\n"; fflush(stdout);
	for(int i=0;i<number_of_lasers;i++)
	{
...
void MrIcpPlugin::Main()
{
	...
	while(1) 
	{
		...
		BuildMap();		          // Launch the MrIcpPlugin on two laser scans
 ...
 *  specific handler
 */
int MrIcpPlugin::ProcessMessage(QueuePointer& resp_queue, player_msghdr * hdr, void * data)
{
 ...
 */
int MrIcpPlugin::HandleData(QueuePointer& resp_queue, player_msghdr * hdr, void * idata)
{
...
int MrIcpPlugin::HandleConfigs(QueuePointer &resp_queue,player_msghdr * hdr,void * data)
{
	// Handle Position REQ
...
int  MrIcpPlugin::HandleCommands(QueuePointer& resp_queue,player_msghdr * hdr,void * data)
{
...
int MrIcpPlugin::ProcessMapInfoReq(QueuePointer& resp_queue,player_msghdr * hdr,void * data)
{
...
int MrIcpPlugin::ProcessMapDataReq(QueuePointer& resp_queue,player_msghdr * hdr,void * data)
{
...
// of the laser
int MrIcpPlugin::InRange(double angle,int laser_index)
{
...
Point MrIcpPlugin::TransformToGlobal(Point p,Pose pose)
{
	// Rotate + Translate
	Point temp = p;
...
Pose MrIcpPlugin::TransformToGlobal(Pose p,Pose pose)
{
	// Rotate + Translate
	Point temp = p.p;
...
Point MrIcpPlugin :: ConvertPixel(Point  p) 
{
...
Point MrIcpPlugin :: ConvertToPixel(Point p) 
{
...
mapgrid_t MrIcpPlugin::ComputeRangeProb(double range,bool free)
{
	double bad_range=0.;
	mapgrid_t prob;
...
void MrIcpPlugin::AddToMap(vector<Point> laser_data,Pose pose)
{
...
void MrIcpPlugin::GenerateLocalMap(Pose pse)
{
...
void MrIcpPlugin::ConnectPatches()
{
	int patch_number=0;
	gchar * patch;
...
extern "C"  // Mod plugin
{
  ...
  {
    MrIcpPlugin_Register(table);  // Mod plugin
    return(0);  // Mod plugin
  }
}
